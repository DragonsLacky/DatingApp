...
public class MessageHub : Hub
{
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto)
    {
        ...
        else
        {
            ...
            if (connections != null)
            {
                await _presenceHub.Clients.Clients(connections).SendAsync("NewMessageNotification", new { username = sender.UserName, knownAs = sender.KnownAs });
            }
        }
    ...
    {
        return string.Compare(caller, other) < 0 ? $"{caller}-{other}" : $"{other}-{caller}";
    }
